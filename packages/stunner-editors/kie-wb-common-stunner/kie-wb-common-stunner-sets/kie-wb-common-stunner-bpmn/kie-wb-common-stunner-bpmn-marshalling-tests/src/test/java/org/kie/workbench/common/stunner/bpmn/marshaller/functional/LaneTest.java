...
public class LaneTest extends BPMNDiagramMarshallerBaseTest {
    ...
    private BaseElement getNodeById(Definitions diagram, String id) {

        for (BaseElement node : getNodesAsBaseElements(diagram.getProcess())) {
    ...
    private void checkLaneMarshalling(String nodeID, String... nodesInsideLane) throws Exception {
        Definitions initialDiagram = marshaller.parseDefinitions(loadStream(BPMN_LANE_FILE_PATH));
        final int AMOUNT_OF_NODES_IN_DIAGRAM = getNodesAsBaseElements(initialDiagram.getProcess()).size();
        String resultXml = marshaller.marshall(initialDiagram);

        Definitions marshalledDiagram = marshaller.parseDefinitions(getStream(resultXml));
        assertEquals(AMOUNT_OF_NODES_IN_DIAGRAM, getNodesAsBaseElements(initialDiagram.getProcess()).size());

        Lane nodeBeforeMarshalling = (Lane) getNodeById(initialDiagram, nodeID);
        Lane nodeAfterMarshalling = (Lane) getNodeById(marshalledDiagram, nodeID);
        assertThat(nodeAfterMarshalling).isEqualTo(nodeBeforeMarshalling);
