...
public abstract class AbstractDataTypeCache {
    ...
    public void extractFromItem(View view) {
        ...
        if (definition instanceof DataObjectReference) {
            ...
            allDataTypes.addAll(getDataTypes(adhoc.getProcessData().getProcessVariables(), false));
        } else if (definition instanceof Process) {
            Process diagram = (Process) definition;
            allDataTypes.addAll(getDataTypes(diagram.getProcessData().getProcessVariables(), false));
        } else if (definition instanceof EmbeddedSubprocess) {
            EmbeddedSubprocess embeddedSubprocess = (EmbeddedSubprocess) definition;
            allDataTypes.addAll(getDataTypes(embeddedSubprocess.getProcessData().getProcessVariables(), false));
        } else if (definition instanceof EventSubprocess) {
            EventSubprocess eventSubprocess = (EventSubprocess) definition;
            allDataTypes.addAll(getDataTypes(eventSubprocess.getProcessData().getProcessVariables(), false));
            ...
            allDataTypes.addAll(getDataTypes(multipleInstanceSubprocess.getProcessData().getProcessVariables(), false));
